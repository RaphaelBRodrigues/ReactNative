...
const getNeighbors = (board, row, column) => {
    ...
    rows.forEach(r => {
        columns.forEach(c => {
            const different = r !== row || c !== columns;
            const validRow = r >= 0 && r < board.length;
            const validColumn = c >= 0 && c < board[0].length;
            if (validRow && validColumn && different) {
...
const hasExplosion = (board) => {return fields(board).filter(field => field.exploded).length > 0};
const pending = field => {
    return (field.mined && !field.flagged) || (!field.mined && !field.opened)
}

const wonGame = board => {
    fields(board).filter(pending).length === 0;
}
...
export { createMinedBoard , cloneBoard , openField , hasExplosion , wonGame , showMines , flagUsed , invertFlag };